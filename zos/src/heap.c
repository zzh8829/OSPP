...
void heap_init()
{
	kp("Heap Initializing");
	kheap = heap_create(KHEAP_START, KHEAP_START+KHEAP_INITIAL_SIZE, KHEAP_START+KHEAP_MAX_ADDRESS, 0, 0);
}

uint32_t physical_address(void* virt)
{
	page_t *page = get_page((uint32_t)virt, 0, kernel_directory);
	return page->frame*0x1000 + ((uint32_t)virt&0xFFF);
}

uint32_t _kmalloc(uint32_t sz, int align, uint32_t *phys)
{
	if (kheap != 0)
	{
...
uint32_t kzmalloc(uint32_t size)
{
	uint32_t addr = _kmalloc(size,0,NULL);
...
{
	uint32_t addr = _kmalloc(size,1,NULL);
	memset((void*)addr,0,size);
	return addr;
}
...
{
	uint32_t addr = _kmalloc(size,1,phys);
	memset((void*)addr,0,size);
	return addr;
}
...
static int32_t find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
	...
	{
		header_t *header = (header_t *)ordered_array_get(&heap->index,iterator);
...
heap_t *heap_create(uint32_t start, uint32_t end_addr, uint32_t max, uint8_t supervisor, uint8_t readonly)
{
	...
	heap->index = ordered_array_place( (void*)start, HEAP_INDEX_SIZE, (less_than_f)&header_t_less_than);
...
void *heap_alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
	...
	if (iterator == -1) // If we didn't find a suitable hole
	{
		...
		while (iterator < heap->index.size)
		{
			uint32_t tmp = (uint32_t)ordered_array_get(&heap->index,iterator);
			if (tmp > value)
			{
				value = tmp;
				idx = iterator;
		...
		else
		{
			...
			header_t *header = ordered_array_get(&heap->index,idx);
			header->size += new_length - old_length;
			// Rewrite the footer.
	...
	header_t *orig_hole_header = (header_t *)ordered_array_get(&heap->index,iterator);
	...
	else
	{
		...
		ordered_array_remove(&heap->index,iterator);
	}
...
void heap_free(void *p, heap_t *heap)
{
		...
		test_header->is_hole)
	{
		...
		int32_t iterator = 0;
		while ( (iterator < heap->index.size) &&
				(ordered_array_get(&heap->index,iterator) != (void*)test_header) )
			iterator++;
